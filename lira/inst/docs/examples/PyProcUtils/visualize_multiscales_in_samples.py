...
#   Initialize output movie files:
    outratfilename_list, outctsfilename_list = [], []
    outratfile_list, outctsfile_list = [], []
    for ilevel in range(powerof2):
        this_cts_filename = OutCtsMovieFitsFileStem+'_Lvl'+str(ilevel)+'.fits'
        this_rat_filename = OutRatMovieFitsFileStem+'_Lvl'+str(ilevel)+'.fits'
        outctsfilename_list.append(this_cts_filename)
        outratfilename_list.append(this_rat_filename)
        this_cts_fileo = pyfits.open(this_cts_filename,mode='append') #or update.
        this_rat_fileo = pyfits.open(this_rat_filename,mode='append')
        outctsfile_list.append( this_cts_fileo )
        outratfile_list.append( this_rat_fileo )

#    1) For this image:
    print ' InMCMCParams[0:1][image_in]: ', \
            InMCMCParams
    for image_in in range(naxis3): # Begin Loop over all sample images/movie frames
        this_ms_ag = n.zeros((powerof2,naxis2,naxis1))
        this_ratio = n.ones((powerof2,naxis2,naxis1))
        OutMCMCImagCount = n.zeros((powerof2,naxis2,naxis1))
        OutMCMCImagRatio = n.ones((powerof2,naxis2,naxis1))
        print '* image_in: ', image_in,' InMCMCParams[0:1][image_in]: ', \
...
##      dim = src->nrow;
        thisdim = dim
##      for(level=0; level < ms->power; level++){ 
        for level in range(0,powerof2-1):
##        dim /= 2.0;                      /* Loop over  Aggregation Levels*/
            thisdim /= 2.0
##          for(i=0; i < dim; i++){
            for i in range(0,thisdim):
##              for(j=0; j < dim; j++){
                for j in range(0,thisdim):
##                     sum = 0;
                    sum = 0.0
##                  for(k=0; k<2; k++)
                    for k in range(0,2):
...
##                        sum += ms->ag[level][2*i+k][2*j+l];
                            try:
#                                print 'indices: i j  k l:', i,j,k,l,' new i, j:', 2*i+k, 2*j+l
                                sum += this_ms_ag[level][2*j+l][2*i+k]
                            except:
                                print 'indices: i j  k l:', i,j,k,l,' new i, j:', 2*i+k, 2*j+l
                                print 'OUT OF BOUNDS!!!'
                                raise IndexError
...
#                   end-for-k
                    for kk in range (0,2):
                        for ll in range(0,2):
                            this_ms_ag[level+1][2*j+ll][2*i+kk] = sum
                            this_ratio[level  ][2*j+ll][2*i+kk] = \
                            this_ms_ag[level  ][2*j+ll][2*i+kk] / \
                            this_ms_ag[level+1][2*j+ll][2*i+kk]
#                       end-for-ll
#                   end-for-kk
...
#          
        ...
        for level in range(0,powerof2):
            for i in range(0,dim):
                for j in range(0,dim):
##                  src->img[(i+spin_row)%dim][(j+spin_col)%dim] = exp( ms->ag[0][i][j] );
                    OutMCMCImagCount[level] \
                      [ n.mod( (j+this_spin_col),dim) ][ n.mod( (i+this_spin_row),dim) ] = \
                      this_ms_ag[level][j][i]
                    OutMCMCImagRatio[level] \
                      [ n.mod( (j+this_spin_col),dim) ][ n.mod( (i+this_spin_row),dim) ] = \
                      this_ratio[level][j][i]
#               end-for-j
#           end-for-i
...
#       IE APPEND the various level data to the appropriate FITS files!
        for ilevel in range(powerof2):
            outctsfile_list[ilevel].append(OutMCMCImagCount[level])
            outratfile_list[ilevel].append(OutMCMCImagRatio[level])


# Close it all up:

    for ilevel in range(powerof2):
        outratfile_list[ilevel].close()
        outctsfile_list[ilevel].close()
