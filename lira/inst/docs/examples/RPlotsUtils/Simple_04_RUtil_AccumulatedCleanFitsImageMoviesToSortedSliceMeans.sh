...
r --vanilla --no-restore <<EOF > Simple_04_RUtil_AccumulatedCleanFitsImageMoviesToSortedSliceMeans.logtst2_2975iters_05slicesSortOnMSCnts
...
outfile = "../PostProcessedFiles/Simple_04_PoisDatons_128x128FaintE_conv_Gauss2d_Sig_1.5_17x17.20120427a_iter60000_thin20_0to2.05slicesSortOnMSCnts.2975.fits"
...
infileParDat0 = '../PostProcessedFiles/Simple_04_PoisDatons_128x128FaintE_conv_Gauss2d_Sig_1.5_17x17.20120427a_iter60000_thin20_Strt0to2_viaFits.RenormSumStat.par'
...
## Usual order: a bunch of renormed hyper-pars; renormedMSCounts, renormed BkgScale,
##  StatSumHyp,  StatSumMSB,  StatSumTot,
##  iSortMSCts, iSortSSMSB, iSortSSHyp,  iSortSSTot
####################################################

## Example: want to sort on smoothing, so choose:
##ichooseSortSumStatVec = numincols - 1
## Or: want to sort on MSCts and BkgScale, so choose:
##ichooseSortSumStatVec = numincols - 2
## Or, if want just MSCts:
ichooseSortSumStatVec = numincols - 3

iSortSumStatVec <- ParDat0[[ichooseSortSumStatVec]]
...
for (lslice in 1:(numslices-1)){
    for (kl in 1:numperslice){
        ks = iSortSumStatVec[( kl + ( (lslice-1)*numperslice ) )]
