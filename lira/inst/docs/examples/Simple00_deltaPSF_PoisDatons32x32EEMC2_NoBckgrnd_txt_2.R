...
img <- lira(
  ...
  out.file = "RunOutputs/PoisDatons32x32EEMC2_NoBckgrnd_2.out",
  param.file="RunOutputs/PoisDatons32x32EEMC2_NoBckgrnd_2.param",
  mcmc=TRUE, , fit.bkg.scale = FALSE,
  max.iter=1000, thin=1,
  burn=0,
  alpha.init=c(3,4,5,6,7))

pdf("RunOutputs/PoisDatonsEEMC2_32x32testE_NoBckgrnd_images_2.pdf", width=8, height=4.25)
par(mfrow=c(1,2))
initial.image <- array(
  scan("exampledata/PoisDatonsEEMC2_32x32testE.090120c.txt"), 
  c(32,32))
