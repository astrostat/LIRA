...
;
;syntax
;	liraimages,lstr0,lstr1,lstr2,lstr3,lstr4,lstr5,outroot=outroot
;
;parameters
;	lstr0	[INPUT; required] output from RD_LIRAOUT()
;	lstr1	[INPUT; optional] LIRA outputs from another run, for comparison
;	lstr2	[INPUT; optional] LIRA outputs from another run, for comparison
;	lstr3	[INPUT; optional] LIRA outputs from another run, for comparison
;	lstr4	[INPUT; optional] LIRA outputs from another run, for comparison
;	lstr5	[INPUT; optional] LIRA outputs from another run, for comparison
;	lstr6	[INPUT; optional] LIRA outputs from another run, for comparison
;	lstr7	[INPUT; optional] LIRA outputs from another run, for comparison
;
;keywords
...
;		  (not implemented yet) OUTROOT_##_iterimg.fits : images from all iterations
;		  OUTROOT_##_data : ps or jpg of the data image
;		  OUTROOT_##_data_base : ps or jpg of the data image with contours from the baseline
...
pro liraimages,lirastr,lstr1,lstr2,lstr3,lstr4,lstr5,lstr6,lstr7,outroot=outroot

;	usage
ok='ok' & np=n_params() & nl=n_elements(lirastr) & nlt=n_tags(lirastr)
if np eq 0 then ok='insufficient parameters' else $
 if nl eq 0 then ok='LIRASTR is not defined -- see rd_liraout()' else $
  if nlt eq 0 then ok='LIRASTR must be a structure'
if ok ne 'ok' then begin
  print,'Usage: liraimages,lirastr,lstr1,lstr2,lstr3,lstr4,lstr5,$
...
for j=0,numstr-1 do begin	;{for each of lstrX
  ...
  ;avimg=total(iterimg,3,/nan)/float(niter)
  ;sdimg=0.*avimg
  ...
  mwrfits,lstrX.AVGIMG,root+'_'+string(j,'(i2.2)')+'_avgmscomp.fits',lstrX.HEADER,/create

  ;	FITS image of snr
  mwrfits,lstrX.SNRMAP,root+'_'+string(j,'(i2.2)')+'_snr.fits',lstrX.HEADER,/create
  ...
  plot_contour_over_image,lstrX.AVGIMG,title='AvgMScomp',windowID=3,/inlog
